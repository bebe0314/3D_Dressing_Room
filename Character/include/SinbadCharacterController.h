...
extern int mySelectItem;
const uint m_Width = 640;
const uint m_Height = 480;

Vector3 newHandPos;
Vector3 newlHandPos;
SceneNode* mSceneNode0;
SceneNode* mSceneNode1;
/*
SceneNode* mSceneNode0;
SceneNode* mSceneNode1;
SceneNode* mSceneNode2;
SceneNode* mSceneNode3;
SceneNode* mSceneNode4;
SceneNode* mSceneNode5;
SceneNode* mSceneNode6;
SceneNode* mSceneNode7;
SceneNode* mSceneNode8;
SceneNode* mSceneNode9;
SceneNode* mSceneNode10;
SceneNode* mSceneNode11;
SceneNode* mSceneNode12;
SceneNode* mSceneNode13;
SceneNode* mSceneNode14;
SceneNode* mSceneNode15;
*/
...
public:
	...
	static void XN_CALLBACK_TYPE GestureRecognized(xn::GestureGenerator& generator, const XnChar* strGesture, const XnPoint3D* pIDPosition, const XnPoint3D* pEndPosition, void* pCookie)
	{
		...
		else if (strcmp(strGesture, "RaiseHand") == 0)
		{
			...
			This->m_HandsGenerator.StartTracking(*pEndPosition);
		}
	}
	...
	{
		SinbadCharacterController* This = (SinbadCharacterController*)pCookie;
		printf("New Hand: %d detected!\n", nId);
		printf("%f, %f, %f\n", pPosition->X, pPosition->Y, pPosition->Z);
		This->m_GestureGenerator.AddGesture("Click", NULL);
	}
	...
	static void XN_CALLBACK_TYPE HandUpdate(xn::HandsGenerator& generator, XnUserID nId, const XnPoint3D* pPosition, XnFloat fTime, void* pCookie)
	{
		SinbadCharacterController* This = (SinbadCharacterController*)pCookie;
		XnPoint3D wPos;
		This->m_DepthGenerator.ConvertRealWorldToProjective(1, pPosition, &wPos);

		printf("%f, %f, %f\n", wPos.X, wPos.Y, wPos.Z);
	}
	
	static void XN_CALLBACK_TYPE HandDestory(xn::HandsGenerator& generator, XnUserID nId, XnFloat fTime, void* pCookie)
	{
		SinbadCharacterController* This = (SinbadCharacterController*)pCookie;
		printf("Lost Hand: %d", nId);
		This->m_GestureGenerator.AddGesture("RaiseHand", NULL);
		This->m_GestureGenerator.RemoveGesture("Click");
	}
	...
	*/
	...
	XnStatus initPrimeSensor()
	{
		...
		/*
		//==================================================My mouse start==================================================
		SinbadCharacterController::sGestureToPress = "Click";
		SinbadCharacterController::sGestureToUse = "RaiseHand";
		m_GestureGenerator.RegisterGestureCallbacks(SinbadCharacterController::GestureRecognized, NULL, this, m_hGestureCallbacks);
		m_GestureGenerator.AddGesture("RaiseHand", NULL);
		m_HandsGenerator.RegisterHandCallbacks(SinbadCharacterController::HandCreate, SinbadCharacterController::HandUpdate, SinbadCharacterController::HandDestory, this, m_hHandCallbacks);
		//==================================================My mouse  end ==================================================
		*/
#if SHOW_DEPTH
	...
	{
#if SHOW_DEPTH || SHOW_BAR
		TexturePtr texture1 = TextureManager::getSingleton().getByName("MyImageTexture");
		// Get the pixel buffer
		HardwarePixelBufferSharedPtr pixelBuffer1 = texture1->getBuffer();

		// Lock the pixel buffer and get a pixel box
		pixelBuffer1->lock(HardwareBuffer::HBL_DISCARD); 
		const PixelBox& pixelBox1 = pixelBuffer1->getCurrentLock();

		unsigned char* pDest = static_cast<unsigned char*>(pixelBox1.data);
		...
		/////////////////////////////////////////////////////////////////////////
		const XnUInt8* pImageMap = m_ImageGenerator.GetImageMap();
		/////////////////////////////////////////////////////////////////////////

		for (size_t j = 0; j < m_Height; j++)
		{
			pDest = static_cast<unsigned char*>(pixelBox1.data) + j*pixelBox1.rowPitch*4;
#if SHOW_DEPTH
			for(size_t i = 0; i < m_Width; i++)
#elif SHOW_BAR
			for(size_t i = 0; i < 50; i++)
#endif
			{
				...
				*pDest++ = (unsigned int)pImageMap[(j*m_Width+fixed_i)*3+2];  // blue
				*pDest++ = (unsigned int)pImageMap[(j*m_Width+fixed_i)*3+1];  // green
				*pDest++ = (unsigned int)pImageMap[(j*m_Width+fixed_i)*3+0];  // red
				pDest++;												// alpha
...
private:
	...
	void setupBody(SceneManager* sceneMgr)
	{
		...
		mBodyNode->setScale(Vector3(0.001,0.001,0.001));
	...
	Vector3 getMyPosition(const Ogre::String& modelBoneName)
	{
		
		Skeleton* skel = mBodyEnt->getSkeleton();
		Ogre::Bone* bone = skel->getBone(modelBoneName);
		Vector3 worldpos = mBodyNode->_getDerivedPosition() + mBodyNode->_getDerivedOrientation() * bone->_getDerivedPosition();
	...
	void PSupdateBody(Real deltaTime)
	{
		...
		if (pUserSkel.IsTracking(m_candidateID))
		{
			...
			rootPos = getMyPosition("Root");
			stomachPos = getMyPosition("Stomach");
			waistPos = getMyPosition("Waist");
			chestPos = getMyPosition("Chest");
			humerusLPos = getMyPosition("Humerus.L");
			humerusRPos = getMyPosition("Humerus.R");
			ulnaLPos = getMyPosition("Ulna.L");
			ulnaRPos = getMyPosition("Ulna.R");
			thighLPos = getMyPosition("Thigh.L");
			thighRPos = getMyPosition("Thigh.R");
			calfLPos = getMyPosition("Calf.L");
			calfRPos = getMyPosition("Calf.R");
			handLPos = getMyPosition("Hand.L");
			handRPos = getMyPosition("Hand.R");
			footLPos = getMyPosition("Foot.L");
			footRPos = getMyPosition("Foot.R");
			
			mSceneNode0->setPosition(handLPos.x, handLPos.y+3, 0);
			mSceneNode1->setPosition(handRPos.x, handRPos.y+3, 0);
			printf("%f, %f, %f\n", handLPos.x, handLPos.y+3, 0);
			//printf("%f, %f, %f\n", handRPos.x, handRPos.y+3, 0);
			...
			{			 
				pUserSkel.GetSkeletonJointPosition(m_candidateID, XN_SKEL_TORSO, torsoPos);
				pUserSkel.GetSkeletonJointPosition(m_candidateID, XN_SKEL_LEFT_HAND, rightHandPos);
				pUserSkel.GetSkeletonJointPosition(m_candidateID, XN_SKEL_RIGHT_HAND, leftHandPos);
				Vector3 newPos;
				...
				//==============my mouse2==============
				newHandPos.x = -rightHandPos.position.X/100;
				newHandPos.y = rightHandPos.position.Y/100;
				newHandPos.z = -rightHandPos.position.Z/100;

				newlHandPos.x = -leftHandPos.position.X/100;
				newlHandPos.y = leftHandPos.position.Y/100;
				newlHandPos.z = -leftHandPos.position.Z/100;
				//==============my mouse2==============
	...
	void updateBody(Real deltaTime)
	{
		...
		if (mKeyDirection != Vector3::ZERO && mBaseAnimID != ANIM_DANCE)
		{
			...
			Real yawAtSpeed = yawToGoal / Ogre::Math::Abs(yawToGoal) * deltaTime * TURN_SPEED;
	...
	void fadeAnimations(Real deltaTime)
	{
		for (int i = 0; i < NUM_ANIMS; i++)
		{
			if (mFadingIn[i])
			{
				...
				mAnims[i]->setWeight(Ogre::Math::Clamp<Real>(newWeight, 0, 1));
